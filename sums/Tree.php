...
class Tree
{
    ...
    public function getView($myid, $itemtpl, $selectedids = '', $disabledids = '', $wraptag = 'ul', $wrapattr = '', $deeplevel = 0)
    {
        ...
        if ($childs) {
            foreach ($childs as $value) {
                ...
                $childlist = $childdata ? "<dl class='layui-nav-child'> " . $childdata . "</dl>" : "";
                $childlist = strtr($childlist, array('@class' => $childdata ? 'last' : ''));$var=url($value['@name']);
				$value = array(
                    '@childlist' => $childlist,
     ...
     * @return array
     */
    public function getChildlist($myid, $withself = false)
    {
        $newarr = [];
        foreach ($this->arr as $value) {
            if (!isset($value['id'])) {
            ...
            if ($value[$this->pidname] == $myid) {
                ...
                $newarr[] = $value;
                $newarr = array_merge($newarr, $this->getChildlist($value['id']));
            } elseif ($withself && $value['id'] == $myid) {
                $newarr[] = $value;
            }
        }
    ...
    public function getTreeAtom($data = [], $field = 'name')
    {
        $arr = [];
        foreach ($data as $k => $v) {
            $childlist = isset($v['childlist']) ? $v['childlist'] : [];
            ...
            if ($childlist) {
                $arr = array_merge($arr, $this->getTreeAtom($childlist, $field));
            }
        }
        return $arr;
    }	
