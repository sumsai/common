...
class Auth extends \sums\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
        ...
        if (Config::get('sower.login_failure_retry') && $admin->loginfailure >= 10 && time() - $admin->updatetime < 86400) {
    ...
    {
        $admin = Admin::first(intval($this->id));
        if (!$admin) {
            return true;
        }
        $admin->token = '';
    ...
    public function getUserInfo($uid = null)
    {
        ...
        return $uid != $this->id ? Admin::first(intval($uid)) : Session::get('admin');
    }
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        // 取出所有分组
        $groupList = \app\admin\model\AuthGroup::where(['status' => 'normal'])->dataset();
        $objList = [];
        foreach ($groups as $K => $v) {
            if ($v['rules'] === '*') {
                $objList = $groupList;
    ...
    public function navone($params = [])
    {
        ...
        // 必须将结果集转换为数组
        $ruleList = \app\admin\model\AuthRule::where('status', 'normal')->where('ismenu', 1)->order('weigh', 'desc')->cache("__menu__")->dataset()->toArray();
        ...
        $menu = Tree::instance()->getView(0, '<li data-name="id" class="layui-nav-item @class">
              <a   @url  lay-tips="@title" lay-direction="2">
                <i class="@icon"></i>
                <cite>@title</cite>
              </a>
			  @childlist
    ...
    public function quanxian($params = [])
    {
        $module = request()->app();
        $userRule = $this->getRuleList();
        $ruleList = \app\admin\model\AuthRule::where('status', 'normal')->where('ismenu', 1)->order('weigh', 'desc')->cache("__menu__")->dataset()->toArray();
        ...
        $ruleTree = new Tree();
        Tree::instance()->init($ruleList);
		$menu = Tree::instance()->getChildren($myid=181, $withself = true);	
        return [$menu];
    }
    ...
    public function addAuth($params = [])
    {
        ...
        $ruleTree = new Tree();
        Tree::instance()->init($ruleList);
		$list = Tree::instance()->getChildlist($myid=181, $withself = true);
		 foreach ($list as $k => $val) {
			$dataList[$k]=$val['title'];
        }
        return [$dataList];
    }	
    ...
    public function editAuth($params = [])
    {
       ...
       $this->model =  new AuthRule;
        // 必须将结果集转换为数组
        $ruleList = $this->model->order('weigh', 'desc')->order('id', 'asc')->dataset()->toArray();
        foreach ($ruleList as $k => &$v) {
            $v['title'] = SoLan($v['title']);
            $v['remark'] = SoLan($v['remark']);
        }
        unset($v);
        Tree::instance()->init($ruleList);
        $this->rulelist = Tree::instance()->getTreeList(Tree::instance()->getTreeArray(0), 'title');
        $ruledata = [0 => SoLan('None')];
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $select_id = $selected ? $selected['id'] : 0;
        $menu = $nav = '';
        if (Config::get('sower.multiplenav')) {
            $topList = [];
